...
export default function ProgressSidebar({ step = 1 }) {
  const steps = [
    { label: "Project Documents" },
    { label: "ITMO Issurance Request" },
  ...
  return (
    <aside className="w-80 bg-white rounded-xl shadow p-6 mr-8 h-fit">
      ...
      <div className="text-gray-600 text-sm mb-6">Your Current Progress</div>
        ...
        {steps.map((item, idx) => {
          ...
          if (idx + 1 < step) {
            return (
              <div key={idx} className="flex items-center gap-3">
                ...
                <button type="button" className="flex-1 text-left px-4 py-2 rounded font-semibold border whitespace-nowrap bg-green-50 text-green-700 border-green-200">{item.label}</button>
              </div>
            );
          }
          // Current step
          if (idx + 1 === step) {
            return (
              <div key={idx} className="flex items-center gap-3">
                ...
                <button type="button" className="flex-1 text-left px-4 py-2 rounded font-semibold border whitespace-nowrap bg-[#32623A] text-white border-[#32623A]">{item.label}</button>
              </div>
            );
          }
          // Future steps
          return (
            <div key={idx} className="flex items-center gap-3">
              ...
              <button type="button" className="flex-1 text-left px-4 py-2 rounded font-semibold border whitespace-nowrap bg-white text-gray-700 border-gray-300">{item.label}</button>
            </div>
          );
        })}
      </div>
