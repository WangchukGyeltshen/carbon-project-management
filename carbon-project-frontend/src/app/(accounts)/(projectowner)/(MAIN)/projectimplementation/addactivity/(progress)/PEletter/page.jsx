...
export default function Letter() {
  ...
  useEffect(() => {
    // Replace this with your real backend fetch
    setTimeout(() => {
      setFile({
        name: "Authorization_Letter.pdf", 
        url: "/files/Positive_Examination_Letter.pdf",
        size: 1.2 * 1024 * 1024, // bytes (1.2MB)
      });
    }, 500);
  }, []);
      ...
      <Sidebar/>
        ...
        <main className="flex-1 overflow-auto p-8">
          <div className="flex gap-4">
            <ProgressSidebar step={4} />
            <div className="w-full">
              <div className="text-2xl font-bold mb-1 text-black">Letter of Positive Examination</div>
                  ...
                  <div className="flex-1">
                    {file.name && (
                      <a
                        href={file.url}
                        download={file.name}
                        className="text-green-700 underline hover:text-green-900 text-base font-medium"
                      >
                        {file.name}
                      </a>
                    )}
                    {file.size && (
                      <div className="text-gray-500 text-sm mt-1">{formatSize(file.size)}</div>
                    )}
                  </div>
                  <a
                    href={file.url}
                    download={file.name}
                    className="ml-6 flex items-center justify-center text-gray-700 hover:text-black"
                ...
                <button
                  ...
                  onClick={() => {
                    if (file) {
                      ...
                      router.push("/projectimplementation", { state: { newActivity } });
                    } else {
                      router.push("/projectimplementation");
                    }
                  }}
                >
                  Next
