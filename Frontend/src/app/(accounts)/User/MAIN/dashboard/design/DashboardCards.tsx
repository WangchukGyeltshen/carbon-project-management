...
interface DashboardTotals {
  projects?: TotalsSection;
  monitoringReports?: TotalsSection;
  letterOfIntent?: TotalsSection;
}

export default function DashboardCards({ totals, loading }: { totals?: DashboardTotals; loading: boolean }): JSX.Element {
  if (loading) {
    return (
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 px-8 py-6">
      {[
        ...
        <div key={item.label} className="bg-white border border-[#E5E7EB] rounded-lg shadow-sm p-7 flex flex-col justify-between min-h-[170px]">
